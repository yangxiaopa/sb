...
节点编号优化（《结构分析的建模》powell）P43
#不同的节点编号方式将影响结构刚度矩阵的形式，如书中P43页所示，方式1的结构整体刚度矩阵带宽更窄，在方程求解时，刚度矩阵是逐步改变的，在窄带区域内的零刚
#刚度元素将逐步被非零值填充而窄带区域外的项总是保持为零。刚度矩阵为窄带时，需要更少的计算机内存来存储刚度矩阵，同时求解方程所需要的数值操作量也更小，
#加速大型结构的求解效率更加明显。结构分析的计算机程序通常都对节点重新编号以优化结构刚度矩阵，通过最小化带宽，比如：对节点重新编号、对自由度进行重新编
#号以及某些计算机程序更复杂的方法“调整”结构刚度矩阵，为减少平衡方程的求解时间！
